...
const LobbyParticipantItem = ({ participant: p }) => {
    const dispatch = (0, react_redux_1.useDispatch)();
    const admit = (0, react_1.useCallback)(() => dispatch((0, actions_native_1.setKnockingParticipantApproval)(p.id, true)), [dispatch, p.id]);
    const reject = (0, react_1.useCallback)(() => dispatch((0, actions_native_1.setKnockingParticipantApproval)(p.id, false)), [dispatch, p.id]);
    return (<ParticipantItem_1.default displayName={p.name ?? ''} isKnockingParticipant={true} key={p.id} participantID={p.id}>
