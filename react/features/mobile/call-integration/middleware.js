...
const functions_3 = require("../../base/connection/functions");
const actions_2 = require("../../base/media/actions");
const constants_1 = require("../../base/media/constants");
const functions_4 = require("../../base/media/functions");
...
const CallIntegration = CallKit_1.default || ConnectionService_1.default;
/**
 ...
 * @returns {Function}
 */
CallIntegration && MiddlewareRegistry_1.default.register(store => next => action => {
    switch (action.type) {
        case actionTypes_5._SET_CALL_INTEGRATION_SUBSCRIPTIONS:
        ...
        case actionTypes_1.APP_WILL_UNMOUNT:
            store.dispatch({
                type: actionTypes_5._SET_CALL_INTEGRATION_SUBSCRIPTIONS,
                subscriptions: undefined
            });
            break;
        case actionTypes_3.CONFERENCE_FAILED:
        ...
        case actionTypes_3.CONFERENCE_WILL_LEAVE:
...
function _appWillMount({ dispatch, getState }, next, action) {
    ...
    if ((0, functions_5.isCallIntegrationEnabled)(getState)) {
...
function _conferenceWillJoin({ dispatch, getState }, next, action) {
    ...
    const url = (0, functions_3.getInviteURL)(state);
    const handle = callHandle || url.toString();
    const hasVideo = !(0, functions_4.isVideoMutedByAudioOnly)(state);
    ...
    // it upper-cased.
    conference.callUUID = (callUUID || (0, uuid_1.v4)()).toUpperCase();
    CallIntegration.startCall(conference.callUUID, handle, hasVideo)
        .then(() => {
        const displayName = (0, functions_2.getConferenceName)(state);
        ...
        if (error.code === 'CREATE_OUTGOING_CALL_FAILED') {
            ...
            react_native_1.Alert.alert('Call aborted', 'There\'s already another call in progress.'
...
function _onPerformSetMutedCallAction({ callUUID, muted }) {
    ...
    if (conference && conference.callUUID === callUUID) {
        ...
        (0, functions_1.sendAnalytics)((0, AnalyticsEvents_1.createTrackMutedEvent)('audio', 'call-integration', muted));
        dispatch((0, actions_2.setAudioMuted)(muted, /* ensureTrack */ true));
    }
}
/**
...
function _setAudioOnly({ getState }, next, action) {
    const result = next(action);
    const state = getState();
    if (!(0, functions_5.isCallIntegrationEnabled)(state)) {
        return result;
    }
    const conference = (0, functions_2.getCurrentConference)(state);
    if (conference?.callUUID) {
...
function _syncTrackState({ getState }, next, action) {
    ...
    if (jitsiTrack.isLocal() && conference && conference.callUUID) {
        switch (jitsiTrack.getType()) {
            ...
            case 'video': {
                CallIntegration.updateCall(conference.callUUID, { hasVideo: !(0, functions_4.isVideoMutedByAudioOnly)(state) });
                break;
            }
        }
    }
...
function _updateCallIntegrationMuted(conference, state) {
    const muted = (0, functions_any_1.isLocalTrackMuted)(state['features/base/tracks'], constants_1.MEDIA_TYPE.AUDIO);
