...
ReducerRegistry_1.default.register('features/base/config', (state = _getInitialState(), action) => {
    switch (action.type) {
        case actionTypes_2.UPDATE_CONFIG:
            return _updateConfig(state, action);
        case actionTypes_2.CONFIG_WILL_LOAD:
            return {
                error: undefined,
                /**
        ...
        case actionTypes_1.CONNECTION_PROPERTIES_UPDATED: {
        ...
        case actionTypes_2.LOAD_CONFIG_ERROR:
        ...
        case actionTypes_2.OVERWRITE_CONFIG:
            return {
                ...state,
                ...action.config
            };
...
function _setConfig(state, { config }) {
    ...
    const newState = lodash_1.default.merge({}, config, hdAudioOptions, { error: undefined }, 
    // The config of _getInitialState() is meant to override the config
    // downloaded from the Jitsi Meet deployment because the former contains
    // values that are mandatory.
    _getInitialState());
    (0, functions_2._cleanupConfig)(newState);
    return (0, functions_1.equals)(state, newState) ? state : newState;
}
/**
 ...
 */
function _translateInterfaceConfig(oldValue) {
...
function _translateLegacyConfig(oldValue) {
        ...
        filteredConferenceInfo.forEach(key => {
            ...
            if (key === 'hideRecordingLabel') {
                ...
                newValue.conferenceInfo.autoHide
                    = lodash_1.default.union(newValue.conferenceInfo.autoHide, CONFERENCE_HEADER_MAPPING[key]);
            }
            else {
                newValue.conferenceInfo.alwaysVisible
                    = (newValue.conferenceInfo.alwaysVisible ?? [])
    ...
    if (oldValue.disableRecordAudioNotification) {
        newValue.disabledSounds.unshift('RECORDING_ON_SOUND', 'RECORDING_OFF_SOUND', 'LIVE_STREAMING_ON_SOUND', 'LIVE_STREAMING_OFF_SOUND');
    }
    ...
    if (oldValue.disableProfile) {
        newValue.toolbarButtons = (newValue.toolbarButtons || constants_2.TOOLBAR_BUTTONS)
            .filter((button) => button !== 'profile');
    }
    (0, functions_2._setDeeplinkingDefaults)(newValue.deeplinking);
    return newValue;
}
/**
...
function _updateConfig(state, { config }) {
    const newState = lodash_1.default.merge({}, state, config);
    (0, functions_2._cleanupConfig)(newState);
    return (0, functions_1.equals)(state, newState) ? state : newState;
}
