...
exports.isTopPanelEnabled = exports.getScreenshareFilmstripParticipantId = exports.getThumbnailTypeFromLayout = exports.isFilmstripDisabled = exports.isStageFilmstripEnabled = exports.isStageFilmstripTopPanel = exports.isStageFilmstripAvailable = exports.getPinnedActiveParticipants = exports.getActiveParticipantsIds = exports.isFilmstripScrollVisible = exports.getVerticalViewMaxWidth = exports.showGridInVerticalView = exports.isFilmstripResizable = exports.getIndicatorsTooltipPosition = exports.getDisplayModeInput = exports.computeDisplayModeFromInput = exports.getVerticalFilmstripVisibleAreaWidth = exports.calculateThumbnailSizeForTileView = exports.calculateResponsiveTileViewDimensions = exports.calculateNonResponsiveTileViewDimensions = exports.getNumberOfPartipantsForTileView = exports.getTileDefaultAspectRatio = exports.getThumbnailMinHeight = exports.calculateThumbnailSizeForVerticalView = exports.calculateThumbnailSizeForHorizontalView = exports.isVideoPlayable = exports.shouldRemoteVideosBeVisible = exports.isFilmstripVisible = void 0;
...
const functions_4 = require("../connection-indicator/functions");
const functions_5 = require("../shared-video/functions");
...
function isFilmstripVisible(stateful) {
    return (0, functions_2.toState)(stateful)['features/filmstrip'].visible;
}
...
function shouldRemoteVideosBeVisible(state) {
    ...
    // in the filmstrip.
    const participantCount = (0, functions_1.getParticipantCountWithFake)(state);
    let pinnedParticipant;
    const { disable1On1Mode } = state['features/base/config'];
    const { contextMenuOpened } = state['features/base/responsive-ui'];
    return Boolean(contextMenuOpened
        || participantCount > 2
        ...
        || (participantCount > 1
            ...
            && (state['features/toolbox'].visible
                || ((pinnedParticipant = (0, functions_1.getPinnedParticipant)(state))
 ...
 */
function isVideoPlayable(stateful, id) {
    const state = (0, functions_2.toState)(stateful);
    const tracks = state['features/base/tracks'];
    const participant = id ? (0, functions_1.getParticipantById)(state, id) : (0, functions_1.getLocalParticipant)(state);
    ...
    if (isLocal) {
        const isVideoMuted = (0, functions_3.isLocalTrackMuted)(tracks, constants_1.MEDIA_TYPE.VIDEO);
        isPlayable = Boolean(videoTrack) && !isVideoMuted && !isAudioOnly;
    }
    else if (!participant?.fakeParticipant || (0, functions_1.isScreenShareParticipant)(participant)) {
        // remote participants excluding shared video
        const isVideoMuted = (0, functions_3.isRemoteTrackMuted)(tracks, constants_1.MEDIA_TYPE.VIDEO, id);
        isPlayable = Boolean(videoTrack) && !isVideoMuted && !isAudioOnly && (0, functions_4.isTrackStreamingStatusActive)(videoTrack);
    }
    return isPlayable;
}
exports.isVideoPlayable = isVideoPlayable;
/**
 ...
 */
function calculateThumbnailSizeForHorizontalView(clientHeight = 0) {
    const topBottomMargin = 15;
    const availableHeight = Math.min(clientHeight, (interfaceConfig.FILM_STRIP_MAX_HEIGHT || constants_3.DEFAULT_FILMSTRIP_WIDTH) + topBottomMargin);
...
exports.calculateThumbnailSizeForHorizontalView = calculateThumbnailSizeForHorizontalView;
/**
 ...
 * @returns {{local: {height, width}, remote: {height, width}}}
 */
function calculateThumbnailSizeForVerticalView(clientWidth = 0, filmstripWidth = 0, isResizable = false) {
    const availableWidth = Math.min(Math.max(clientWidth - constants_3.VERTICAL_VIEW_HORIZONTAL_MARGIN, 0), (isResizable ? filmstripWidth : interfaceConfig.FILM_STRIP_MAX_HEIGHT) || constants_3.DEFAULT_FILMSTRIP_WIDTH);
    return {
        local: {
            height: Math.floor(availableWidth
                / (interfaceConfig.LOCAL_THUMBNAIL_RATIO || constants_3.DEFAULT_LOCAL_TILE_ASPECT_RATIO)),
            width: availableWidth
        },
        remote: {
            height: isResizable
                ? constants_3.DEFAULT_FILMSTRIP_WIDTH
                : Math.floor(availableWidth / interfaceConfig.REMOTE_THUMBNAIL_RATIO),
            width: availableWidth
        }
    };
}
exports.calculateThumbnailSizeForVerticalView = calculateThumbnailSizeForVerticalView;
/**
 ...
 */
function getThumbnailMinHeight(clientWidth) {
    return clientWidth < constants_3.ASPECT_RATIO_BREAKPOINT ? constants_3.TILE_MIN_HEIGHT_SMALL : constants_3.TILE_MIN_HEIGHT_LARGE;
}
exports.getThumbnailMinHeight = getThumbnailMinHeight;
/**
 ...
 */
function getTileDefaultAspectRatio(disableResponsiveTiles, disableTileEnlargement, clientWidth) {
    if (!disableResponsiveTiles && disableTileEnlargement && clientWidth < constants_3.ASPECT_RATIO_BREAKPOINT) {
        return constants_3.SQUARE_TILE_ASPECT_RATIO;
    }
    return constants_3.TILE_ASPECT_RATIO;
}
exports.getTileDefaultAspectRatio = getTileDefaultAspectRatio;
/**
 ...
 */
function getNumberOfPartipantsForTileView(state) {
    ...
    const numberOfParticipants = (0, functions_1.getParticipantCountWithFake)(state)
        - (iAmRecorder ? 1 : 0)
        - (disableSelfView ? localParticipantsCount : 0);
    return numberOfParticipants;
}
exports.getNumberOfPartipantsForTileView = getNumberOfPartipantsForTileView;
/**
 ...
 */
function calculateNonResponsiveTileViewDimensions(state) {
    const { clientHeight, clientWidth } = state['features/base/responsive-ui'];
    const { disableTileEnlargement } = state['features/base/config'];
    const { columns: c, minVisibleRows, rows: r } = (0, functions_web_1.getNotResponsiveTileViewGridDimensions)(state);
    ...
    if (typeof size === 'undefined') { // The columns don't fit into the screen. We will have horizontal scroll.
        const aspectRatio = disableTileEnlargement
            ...
            : constants_3.TILE_PORTRAIT_ASPECT_RATIO;
...
exports.calculateNonResponsiveTileViewDimensions = calculateNonResponsiveTileViewDimensions;
/**
 * Calculates the dimensions (thumbnail width/height and columns/row) for tile view when the responsive tiles are
 * enabled.
 *
 ...
 * @returns {Object} - The dimensions.
 */
function calculateResponsiveTileViewDimensions({ clientWidth, clientHeight, disableTileEnlargement = false, noHorizontalContainerMargin = false, maxColumns, numberOfParticipants, desiredNumberOfVisibleTiles = constants_3.TILE_VIEW_DEFAULT_NUMBER_OF_VISIBLE_TILES, minTileHeight }) {
    let height, width;
    let columns, rows;
    let dimensions = {
        maxArea: 0
    ...
    for (let c = 1; c <= Math.min(maxColumns, numberOfParticipants, desiredNumberOfVisibleTiles); c++) {
        ...
        const visibleRows = numberOfParticipants <= desiredNumberOfVisibleTiles ? r : Math.floor(desiredNumberOfVisibleTiles / c);
        const size = calculateThumbnailSizeForTileView({
            columns: c,
        ...
        if (size) {
            ...
            let area = Math.round((currentHeight + constants_3.TILE_VERTICAL_MARGIN)
                * (currentWidth + constants_3.TILE_HORIZONTAL_MARGIN)
            ...
            if (!minHeightEnforced) {
                if (area > dimensions.maxArea) {
                    ...
                    && ((oldNumberOfVisibleParticipants > desiredNumberOfVisibleTiles
                        ...
                        || (oldNumberOfVisibleParticipants < numberOfParticipants
                            && numberOfVisibleParticipants <= desiredNumberOfVisibleTiles))) { // If the area of the new candidates and the old ones are equal we prefer the one that will have
...
exports.calculateResponsiveTileViewDimensions = calculateResponsiveTileViewDimensions;
/**
 ...
 */
function calculateThumbnailSizeForTileView({ columns, minVisibleRows, clientWidth, clientHeight, disableResponsiveTiles = false, disableTileEnlargement = false, noHorizontalContainerMargin = false, minTileHeight }) {
    ...
    const viewWidth = clientWidth - (columns * constants_3.TILE_HORIZONTAL_MARGIN)
        - (noHorizontalContainerMargin ? constants_3.SCROLL_SIZE : constants_3.TILE_VIEW_GRID_HORIZONTAL_MARGIN);
    const availableHeight = clientHeight - constants_3.TILE_VIEW_GRID_VERTICAL_MARGIN;
    const viewHeight = availableHeight - (minVisibleRows * constants_3.TILE_VERTICAL_MARGIN);
    ...
    if (disableTileEnlargement) {
        ...
        return {
            ...
            width: aspectRatio * height,
            minHeightEnforced,
            maxVisibleRows: Math.floor(availableHeight / (height + constants_3.TILE_VERTICAL_MARGIN))
        };
    }
    ...
    else if (initialRatio >= constants_3.TILE_PORTRAIT_ASPECT_RATIO) {
        width = initialWidth;
        // eslint-disable-next-line no-negated-condition
    }
    else if (!minHeightEnforced) {
        height = initialWidth / constants_3.TILE_PORTRAIT_ASPECT_RATIO;
        if (height >= minHeight) {
            width = initialWidth;
        }
...
exports.calculateThumbnailSizeForTileView = calculateThumbnailSizeForTileView;
/**
 ...
 */
function getVerticalFilmstripVisibleAreaWidth() {
    ...
    const filmstripMaxWidth = (interfaceConfig.FILM_STRIP_MAX_HEIGHT || constants_3.DEFAULT_FILMSTRIP_WIDTH) + 18;
    return Math.min(filmstripMaxWidth, window.innerWidth);
}
exports.getVerticalFilmstripVisibleAreaWidth = getVerticalFilmstripVisibleAreaWidth;
/**
...
*/
function computeDisplayModeFromInput(input) {
    const { filmstripType, isActiveParticipant, isAudioOnly, isCurrentlyOnLargeVideo, isVirtualScreenshareParticipant, isScreenSharing, canPlayEventReceived, isRemoteParticipant, stageParticipantsVisible, tileViewActive } = input;
    const adjustedIsVideoPlayable = input.isVideoPlayable && (!isRemoteParticipant || canPlayEventReceived);
    // Display video for virtual screen share participants in all layouts.
    if (isVirtualScreenshareParticipant) {
        return constants_3.DISPLAY_VIDEO;
    }
    ...
    if (isScreenSharing) {
        return constants_3.DISPLAY_AVATAR;
    }
    if (!tileViewActive && filmstripType === constants_3.FILMSTRIP_TYPE.MAIN && ((isScreenSharing && isRemoteParticipant)
        || (stageParticipantsVisible && isActiveParticipant))) {
        return constants_3.DISPLAY_AVATAR;
    }
    else if (isCurrentlyOnLargeVideo && !tileViewActive) {
        // Display name is always and only displayed when user is on the stage
        return adjustedIsVideoPlayable && !isAudioOnly ? constants_3.DISPLAY_VIDEO : constants_3.DISPLAY_AVATAR;
    }
...
exports.computeDisplayModeFromInput = computeDisplayModeFromInput;
/**
 ...
 * @returns {Object}
*/
function getDisplayModeInput(props, state) {
    const { _currentLayout, _isActiveParticipant, _isAudioOnly, _isCurrentlyOnLargeVideo, _isVirtualScreenshareParticipant, _isScreenSharing, _isVideoPlayable, _participant, _stageParticipantsVisible, _videoTrack, filmstripType = constants_3.FILMSTRIP_TYPE.MAIN } = props;
    const tileViewActive = _currentLayout === constants_2.LAYOUTS.TILE_VIEW;
...
exports.getDisplayModeInput = getDisplayModeInput;
/**
 ...
 * @returns {string}
 */
function getIndicatorsTooltipPosition(thumbnailType) {
    return constants_3.INDICATORS_TOOLTIP_POSITION[thumbnailType ?? ''] || 'top';
}
exports.getIndicatorsTooltipPosition = getIndicatorsTooltipPosition;
/**
 ...
 * @returns {boolean}
 */
function isFilmstripResizable(state) {
    const { filmstrip } = state['features/base/config'];
    const _currentLayout = (0, functions_web_1.getCurrentLayout)(state);
    return !filmstrip?.disableResizable && !(0, utils_1.isMobileBrowser)()
        && (_currentLayout === constants_2.LAYOUTS.VERTICAL_FILMSTRIP_VIEW || _currentLayout === constants_2.LAYOUTS.STAGE_FILMSTRIP_VIEW);
}
exports.isFilmstripResizable = isFilmstripResizable;
/**
 ...
 * @returns {boolean}
 */
function showGridInVerticalView(state) {
    const resizableFilmstrip = isFilmstripResizable(state);
    const { width } = state['features/filmstrip'];
    return resizableFilmstrip && ((width.current ?? 0) > constants_3.FILMSTRIP_GRID_BREAKPOINT);
}
exports.showGridInVerticalView = showGridInVerticalView;
/**
 ...
 * @returns {number}
 */
function getVerticalViewMaxWidth(state) {
    const { width } = state['features/filmstrip'];
    const _resizableFilmstrip = isFilmstripResizable(state);
    const _verticalViewGrid = showGridInVerticalView(state);
    let maxWidth = _resizableFilmstrip
        ? width.current || constants_3.DEFAULT_FILMSTRIP_WIDTH
        : interfaceConfig.FILM_STRIP_MAX_HEIGHT || constants_3.DEFAULT_FILMSTRIP_WIDTH;
...
exports.getVerticalViewMaxWidth = getVerticalViewMaxWidth;
/**
...
function isFilmstripScrollVisible(state) {
    ...
    switch (_currentLayout) {
        case constants_2.LAYOUTS.TILE_VIEW:
            ({ hasScroll = false } = state['features/filmstrip'].tileViewDimensions ?? {});
            break;
        case constants_2.LAYOUTS.VERTICAL_FILMSTRIP_VIEW:
        case constants_2.LAYOUTS.STAGE_FILMSTRIP_VIEW: {
            ({ hasScroll = false } = state['features/filmstrip'].verticalViewDimensions);
            break;
        }
        case constants_2.LAYOUTS.HORIZONTAL_FILMSTRIP_VIEW: {
 ...
 */
function getActiveParticipantsIds(state) {
 ...
 */
function getPinnedActiveParticipants(state) {
    const { activeParticipants } = state['features/filmstrip'];
 ...
 * @returns {boolean}
 */
function isStageFilmstripAvailable(state, minParticipantCount = 0) {
    const { activeParticipants } = state['features/filmstrip'];
    const { remoteScreenShares } = state['features/video-layout'];
    const sharedVideo = (0, functions_5.isSharingStatus)(state['features/shared-video']?.status ?? '');
 ...
 * @returns {boolean}
 */
function isStageFilmstripTopPanel(state, minParticipantCount = 0) {
    const { remoteScreenShares } = state['features/video-layout'];
    return isTopPanelEnabled(state)
        && isStageFilmstripAvailable(state, minParticipantCount) && remoteScreenShares.length > 0;
}
exports.isStageFilmstripTopPanel = isStageFilmstripTopPanel;
/**
 ...
 * @returns {boolean}
 */
function isStageFilmstripEnabled(state) {
    const { filmstrip } = state['features/base/config'];
 ...
 * @returns {boolean}
 */
function isFilmstripDisabled(state) {
    const { filmstrip } = state['features/base/config'];
    return Boolean(filmstrip?.disabled);
}
exports.isFilmstripDisabled = isFilmstripDisabled;
/**
 ...
 * @returns {string}
 */
function getThumbnailTypeFromLayout(currentLayout, filmstripType) {
    switch (currentLayout) {
        case constants_2.LAYOUTS.TILE_VIEW:
            return constants_3.THUMBNAIL_TYPE.TILE;
        case constants_2.LAYOUTS.VERTICAL_FILMSTRIP_VIEW:
            return constants_3.THUMBNAIL_TYPE.VERTICAL;
        case constants_2.LAYOUTS.HORIZONTAL_FILMSTRIP_VIEW:
            return constants_3.THUMBNAIL_TYPE.HORIZONTAL;
        case constants_2.LAYOUTS.STAGE_FILMSTRIP_VIEW:
            if (filmstripType !== constants_3.FILMSTRIP_TYPE.MAIN) {
                return constants_3.THUMBNAIL_TYPE.TILE;
            }
            return constants_3.THUMBNAIL_TYPE.VERTICAL;
    }
}
exports.getThumbnailTypeFromLayout = getThumbnailTypeFromLayout;
/**
 ...
 */
function getScreenshareFilmstripParticipantId(state) {
 ...
 * @returns {boolean}
 */
function isTopPanelEnabled(state) {
    const { filmstrip } = state['features/base/config'];
    const participantsCount = (0, functions_1.getParticipantCount)(state);
