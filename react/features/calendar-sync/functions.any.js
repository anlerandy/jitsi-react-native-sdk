...
function _updateCalendarEntries(events) {
    ...
    for (const event of events) {
        ...
        if (entry && _isDisplayableCalendarEntry(entry)) {
            ...
            // multiple calendars.
            const key = js_md5_1.default.hex(JSON.stringify([
    ...
    dispatch((0, actions_1.setCalendarEvents)(Array.from(entryMap.values())
        .sort((a, b) => a.startDate - b.startDate)
        .slice(0, constants_1.MAX_LIST_LENGTH)));
}
exports._updateCalendarEntries = _updateCalendarEntries;
/**
...
function _getURLFromEvent(event, knownDomains) {
    ...
    const schemeRegExp = `${uri_1.APP_LINK_SCHEME}${linkTerminatorPattern}+`;
    ...
    for (const field of fieldsToSearch) {
        if (typeof field === 'string') {
            ...
            if (match) {
                const url = (0, uri_1.parseURIString)(match);
                if (url) {
                    return url.toString();
                }
            }
