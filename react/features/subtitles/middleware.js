...
MiddlewareRegistry_1.default.register(store => next => action => {
    switch (action.type) {
        ...
        case actionTypes_2.TOGGLE_REQUESTING_SUBTITLES: {
        ...
        case actionTypes_2.SET_REQUESTING_SUBTITLES:
...
function _endpointMessageReceived(store, next, action) {
    ...
    if (json.type === JSON_TYPE_TRANSLATION_RESULT && json.language === language) {
        ...
        dispatch((0, actions_any_1.updateTranscriptMessage)(transcriptMessageID, newTranscriptMessage));
    }
        ...
        if (!(json.is_interim && skipInterimTranscriptions)) {
            ...
            (0, functions_1.notifyTranscriptionChunkReceived)(transcriptMessageID, json.language, participant, txt, store);
 ...
 * @returns {void}
 */
function _requestingSubtitlesChange({ getState }, enabled, language) {
    const state = getState();
...
function _setClearerOnTranscriptMessage(dispatch, transcriptMessageID, transcriptMessage) {
    ...
    transcriptMessage.clearTimeOut
        = window.setTimeout(() => dispatch((0, actions_any_1.removeTranscriptMessage)(transcriptMessageID)), REMOVE_AFTER_MS);
}
